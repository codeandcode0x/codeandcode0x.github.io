...
getLatestRelease() {
    ...
    if [ "$CODECI_HTTP_REQUEST_CLI" == "curl" ]; then
        latest_release=$(curl -u 25fdcb22531ff9c59cff423ba6f1deb6dbe864a8:x-oauth-basic -s $codeciReleaseUrl | grep \"tag_name\" | grep -v rc | awk 'NR==1{print $2}' |  sed -n 's/\"\(.*\)\",/\1/p')
    else
        latest_release=$(wget -q --header="Accept: application/json" --header="Authorization: token 25fdcb22531ff9c59cff423ba6f1deb6dbe864a8" -O - $codeciReleaseUrl | grep \"tag_name\" | grep -v rc | awk 'NR==1{print $2}' |  sed -n 's/\"\(.*\)\",/\1/p')
    fi
